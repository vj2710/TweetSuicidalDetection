...
featureSelection <- function(myFrame){
  ...
  for(name in colnames(myFrame)){
    if(nchar(name)==1){
      removeIndex = c(removeIndex, which(colnames(myFrame)==name))
    }
  }
...
extractFeatures <- function(dataset){
  ...
  removeIndex = featureSelection(myFrame)
